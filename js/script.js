...
function checkForm(type) {
  ...
  // Helper function to toggle the invalid class
  function toggleHint(element, condition, hintClass) {
    if (condition) {
      document.querySelector(hintClass).classList.add('invalid');
      isValid = false;
    } else {
      document.querySelector(hintClass).classList.remove('invalid');
    }
  }

  if (type === 'expense') {
    toggleHint(expenseInput, !reg.test(expenseInput.value), '.expenseNameHint');
    toggleHint(expenseAmount, isNaN(expenseAmount.value) || expenseAmount.value.trim() === '', '.expenseAmountHint');
    toggleHint(expenseSelect, expenseSelect.value.trim() === '', '.expenseSelectHint');
  } else if (type === 'goal') {
    toggleHint(savingsInput, !reg.test(savingsInput.value), '.savingsNameHint');
    toggleHint(savingsAmount, isNaN(savingsAmount.value) || savingsAmount.value.trim() === '', '.savingsAmountHint');
    toggleHint(savingsSelect, savingsSelect.value.trim() === '', '.savingsSelectHint');
    toggleHint(monthlyContribution, isNaN(monthlyContribution.value) || monthlyContribution.value.trim() === '', '.savingsContributionsHint');
  }
